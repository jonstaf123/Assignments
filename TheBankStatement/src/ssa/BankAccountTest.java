...
public class BankAccountTest {
	...
	public void TestPrint() {
		...
		String result = df.format(checking.getBalance());
		assertEquals(String.format("%s %3s %-28s %5s ", checking.getAccountType(), checking.getId(),
				checking.getDescription(), result) + checking.getLastChecknbr(), checking.print());

		Savings savings = new Savings();

		savings.setEarnedInterest(1.5);
		DecimalFormat dfs = new DecimalFormat("#.00");
		String savingsResult = dfs.format(savings.getBalance());
		DecimalFormat dfi = new DecimalFormat("#.000");
		String resultInterest = dfi.format(savings.getInterestRate());
		assertEquals(String.format("%s %3s %-28s %5s ", savings.getAccountType(), savings.getId(),
				savings.getDescription(), savingsResult) + "     " + resultInterest, savings.print());
	}
